...
def compute_z2(a1,W2,b2):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    # The shape of W2 is not correct.
    z2 = W2 * a1 + b2
    #########################################
    return z2
...
def compute_dL_da2(a2, y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_da2 = np.zeros(a2.shape)
    try:
        dL_da2[y] = -1 / a2[y, 0]
    except FloatingPointError:
        dL_da2[y] = -999999.
    dL_da2 = np.matrix(dL_da2)
    #########################################
    return dL_da2
...
def compute_dz2_dW2(a1,c):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz2_dW2 = np.repeat(a1.T, c, axis=0)
    #########################################
    return dz2_dW2
...
def compute_dz2_db2(c):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz2_db2 = np.repeat(np.mat(1), c, axis=0)
    #########################################
    return dz2_db2
...
def compute_dz1_dW1(x,h):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz1_dW1 = np.repeat(x.T, h, axis=0)
    #########################################
    return dz1_dW1
...
def compute_dz1_db1(h):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz1_db1 = np.repeat(np.mat("1"), h, axis=0)
    #########################################
    return dz1_db1
...
def compute_gradients(dL_da2, da2_dz2, dz2_dW2, dz2_db2, dz2_da1, da1_dz1, dz1_dW1, dz1_db1):
    ...
    # the 2nd layer 
    # dL_dW2 = np.multiply((dL_da2.T * da2_dz2).T, dz2_dW2)
    # dL_db2 = np.multiply((dL_da2.T * da2_dz2).T, dz2_db2)

    # the 1st layer 
    # dL_dW1 = np.multiply(
    #     np.multiply(
    #         (dL_da2.T * da2_dz2 * dz2_da1).T, da1_dz1), dz1_dW1)
    # dL_db1 = np.multiply(
    #     np.multiply(
    #         (dL_da2.T * da2_dz2 * dz2_da1).T, da1_dz1), dz1_db1)
